...
module.exports = Field.create({
	...
	componentWillUpdate (nextProps) {
		...
		if (this.props.value.etag !== nextProps.value.etag) {
			this.setState({
				removeExisting: false,
				userSelectedFile: null,
			});
	...
	getFilename () {
		const { filename, height, width } = this.props.value;
	...
	renderLightbox () {
		...
		return (
			<Lightbox
				currentImage={0}
				images={[{ src: this.getImageSource() }]}
	...
	renderImagePreview () {
		...
		return (
			...
			>
				<img src={this.getImageSource('admin')} style={{ height: 90 }} />
			</ImageThumbnail>
		);
	},
