...
import Thumbnail from './StorageImagesThumbnail';
...
module.exports = Field.create({
	displayName: 'StorageImagesField',
	statics: {
		type: 'StorageImages',
		getDefaultValue: () => ([]),
	},
	...
	componentWillUpdate (nextProps) {
		...
		const value = _.map(this.props.value, 'etag').join();
		const nextValue = _.map(nextProps.value, 'etag').join();
		if (value !== nextValue) {
			this.setState(this.buildInitialState(nextProps));
		}
	},
	buildInitialState (props) {
		const uploadFieldPath = `StorageImages-${props.path}-${++uploadInc}`;
		const thumbnails = props.value ? props.value.map((img, index) => {
			return this.getThumbnail({
				value: img,
				imageSourceSmall: typeof (img.sizes.admin) !== 'undefined' ? img.sizes.admin.url : '',
